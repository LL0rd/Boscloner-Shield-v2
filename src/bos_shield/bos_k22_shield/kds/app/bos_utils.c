...
uint8_t em410x_uid[11] = "3B0052BE68";
void test_create_buff() {
	ConstructEM410xEmulGraph(em410x_uid, 64);
}

// clear out our graph window
int ClearGraph(int redraw) {
	int gtl = GraphTraceLen;
	memset(BigBuf, 0x00, GraphTraceLen);
	GraphTraceLen = 0;
}
...
void AppendGraph(int redraw, int clock, int bit) {
	...
	for (i = 0; i < (int) (clock / 2); ++i) {
		//BigBuf[GraphTraceLen++] = bit;
		pack_val |= bit << (8*pack_i);
		pack_i++;
		if (pack_i == 4) {
			BigBuf[GraphTraceLen++] = pack_val;
			pack_val = 0;
			pack_i = 0;
		}
	}
	...
	for (i = (int) (clock / 2); i < clock; ++i) {
		//BigBuf[GraphTraceLen++] = bit ^ 1;
		pack_val |= (bit^1) << (8 * pack_i);
		pack_i++;
		if (pack_i == 4) {
			BigBuf[GraphTraceLen++] = pack_val;
			pack_val = 0;
...
/* write a manchester bit to the graph */
void AppendGraph_orig(int redraw, int clock, int bit) {
	int i;
	//set first half the clock bit (all 1's or 0's for a 0 or 1 bit)
	for (i = 0; i < (int) (clock / 2); ++i)
		BigBuf[GraphTraceLen++] = bit;
