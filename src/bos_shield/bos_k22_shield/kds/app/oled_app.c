...
void init_disp_update() {
	...
	reset_display();
	TimerStart(tmr1_dsp, DEF_DSP_UPDATE_TIME);	//user default update time
}
...
/*
 ...
 idle
 */
void bos_print_status() {
	init_printf(NULL, putCharLCD);
	init_disp_update();	//restart display update
	setCursor(0, 0);
	setTextSize(1);
	printf("Boscloner App Status\n");
	printf("PM3 State: %u ",pm3_state);
	switch (pm3_state) {
	case (hid_clone):
		printf("hid clone \n");
		break;
...
void print_received_card_em4100() {
	...
	pdata = get_card_id5(); //get 5 bytes is LF card data read

	clearDisplay();
	setCursor(0, 0);
	setTextSize(1);
	printf("Recieved em4100 data:\n", clone_status);
	setTextSize(2);
	printf("0x:\n");
	for(i=0;i<5;i++){
		printf("%X",*pdata);
