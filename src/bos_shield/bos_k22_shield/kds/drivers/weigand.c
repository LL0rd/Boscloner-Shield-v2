...
//packet with hex5 at id5[0], hex4[1], hex3[2]....msb low bits
void pack_card_id5() {
	uint8_t i;

	for (i = 0; i < 5; i++) {
		if (i < 3) {
			hid.bytes[i] = cardChunk2 >> (i * 8);
		}
		if (i == 3) {
			hid.bytes[i] = cardChunk1;
		}
		if (i == 4) {
			hid.bytes[i] = cardChunk1 >> 8;
		}
	}	//for
}	//function

//load the hid.bytes array with the id values mosty sig byte first (low).
void pack_card_id5_msb() {
	uint8_t i;
	...
	for (i = 0; i < 5; i++) {
		if (i == 0) {
			...
			hid.bytes[i] = cardChunk2 >> 16;
		}
		else if (i == 3) {
			hid.bytes[i] = cardChunk2 >> 8;
		}
		else if (i == 4) {
			hid.bytes[i] = cardChunk2;
		}
...
void sim_pack_card_id5_msb(uint64_t id) {

	uint8_t i;
	//uint64_t id = 0x2006e2309f;
	//HID Prox TAG ID: 2006e2309f (6223) - Format Len: 26bit - FC: 113 - Card: 6223

		for (i = 0; i < 5; i++) {
			if (i == 0) {
				hid.bytes[i] = (id>>32) & 0xff;
			}
			else if (i == 1) {
				hid.bytes[i] = (id>>24) & 0xff;
			}
			else if (i == 2) {
				hid.bytes[i] = (id>>16) & 0xff;
			}
			else if (i == 3) {
				hid.bytes[i] = (id>>8) & 0xff;
			}
			else if (i == 4) {
				hid.bytes[i] = (id) & 0xff;
			}
