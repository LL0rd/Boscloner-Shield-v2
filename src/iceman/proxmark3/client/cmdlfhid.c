...
static bool sendTry(uint8_t fmtlen, uint32_t fc, uint32_t cn, uint32_t delay, uint8_t *bits, bool verbose){
	...
	calcWiegand( fmtlen, fc, cn, bits);	//! this takes teh fc, cn, fmtlen and then puts into weigand. Looks like hi lo values to me.  
...
int CmdHIDDemod(const char *Cmd) {
	...
	//!
	//PrintAndLogEx(NORMAL, "before running demodfsk: hi2:%x hi:%x size:%u lo:%x",
	//		hi2,hi,size,lo);  //no data at this pointexcpet for size.
	...
	//!
	int size_bits;
	size_bits = sizeof(bits);
	PrintAndLogEx(NORMAL, "after running demodfsk: hi2:%x hi:%x size:%u lo:%x, sizebits:%u bits:%x",
				hi2,hi,size,lo,size_bits, bits);


	if (hi2==0 && hi==0 && lo==0) {
		PrintAndLogEx(NORMAL, "DEBUG: Error - HID no values found");
		return 0;
	}
	...
	if (hi2 != 0){ //extra large HID tags
		...
		if ((( hi >> 5) & 1) == 1){//if bit 38 is set then < 37 bit format is used  //!less than 37 bit used.
			...
			//!debug.....
			PrintAndLogEx(NORMAL, "hi:%x  lo: %x", hi, lo);
			PrintAndLogEx(NORMAL, "fc: %u ,fmtLen %u,  idx3: %u", fmtLen, idx3);
			//debug.......
		...
		PrintAndLogEx(NORMAL, "HID Prox TAG ID: %x%08x (%u) - Format Len: %ubit - FC: %u - Card: %u",
...
int CmdHIDRead(const char *Cmd) {
	lf_read(true, 12000);	//! get the modulated signal.  
...
int CmdHIDWiegand(const char *Cmd) {
	...
	uint8_t bits[BITS];
	uint8_t *bs = bits;		//! this is where the wiegand bits are going.  
